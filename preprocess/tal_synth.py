...
class TALSynth(object):
    ...
    def gen_decision(self, n_context, n_query, n_dec):
        ...
        # p = (0.65 / n_dec) * torch.ones(n_dec, ) + 0.35 * torch.distributions.Dirichlet(torch.ones(n_dec, )).sample()
        # balanced situation
...
def plot(sampler, batch):
    ...
    # fig, axs = plt.subplots(sampler.n_dec, 1, figsize=(10, 8))
    #
    # for i, kernel in enumerate(sampler.kernels):
    #     context_mask = (batch.context_d.squeeze(-1) == i).any(dim=0)
    #     query_mask = (batch.query_d.squeeze(-1) == i).any(dim=0)
    #     target_mask = (batch.target_d.squeeze(-1) == i).any(dim=0)
    #
    #     axs[i].scatter(batch.context_x[0, context_mask, 0].numpy(),
    #                    batch.context_y[0, context_mask, 0].numpy(), c='b', label='Context', zorder=10)
    #     axs[i].scatter(batch.query_x[0, query_mask, 0].numpy(),
    #                    batch.query_y[0, query_mask, 0].numpy(), c='r', label='Query', zorder=10)
    #     axs[i].scatter(batch.target_x[0, target_mask, 0].numpy(),
    #                    batch.target_y[0, target_mask, 0].numpy(), c='g', s=100, label='Target', zorder=10, marker='*')
    #
    #     axs[i].set_title(f"GP {i + 1}")
    #     axs[i].legend()
