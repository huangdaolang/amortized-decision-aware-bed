...
class HPOBHandler:
    ...
    def __init__(self, root_dir=os.path.join(this_dir, "hpob-data/"), mode="v3-test", surrogates_dir="saved-surrogates/"):
    ...
    def evaluate(self, bo_method=None, search_space_id=None, dataset_id=None, T=10, seed=None, k=3):
        ...
        # print(y[current_evaluations])
        topk_indices = np.argpartition(y[current_evaluations].reshape(-1), -k)[-k:]
        topk_values = y[current_evaluations][topk_indices]
        max_accuracy_history = [np.sum(topk_values)]
        ...
        for i in range(T):
            ...
            topk_indices = np.argpartition(y[current_evaluations].reshape(-1), -k)[-k:]
            topk_values = y[current_evaluations][topk_indices]
            max_accuracy_history.append(np.sum(topk_values))
            # if max(y) in max_accuracy_history:
            #     break

        # max_accuracy_history+=[max(y).item()]*(T-i-1)
