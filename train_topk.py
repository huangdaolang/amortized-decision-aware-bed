...
from architectures.bo_tnp import DecisionTNP_BO
from utils import *
    ...
    for step in range(cfg.steps):
        ...
        for t in range(T):
            if t == 0:
                ...
                reward_list.append(values_sum.detach())
            ...
            reward = reward_list[-1] - reward_list[-2]
        ...
        true_utility = torch.stack(reward_list, dim=0)
        ...
        if step % cfg.print_freq == 0:
            logger.info(f"Step {step} loss: {loss} one reward trajectory: {rewards[:, 0]} true utility: {true_utility[:, 0]}")
