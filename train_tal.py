...
from architectures.decision_tnp import DecisionTNP
from utils import *
...
def train(cfg):
    ...
    for step in range(cfg.steps):
        ...
        batch = sampler.sample(
            ...
            # n_context=cfg.dataset.n_context,  # if specified, use the specified n_context, otherwise sample
        ...
        for t in range(T+1):  # T+1 steps, including the last decision-making step
            ...
            if decision_out is not None:
                # decision-making part, we first use regression loss, later we use classification loss
                ...
                samples = pred_all.sample(torch.Size([100]))  # [100, batch_size, 2, 1]
                _, max_indices = torch.max(samples.squeeze(-1), dim=2)  # [100, batch_size]

                correct = (max_indices == batch.target_d.flatten()).float()
                r = correct.sum(dim=0)

                target_log_prob_list.append(r)

            if t != 0:
                reward = target_log_prob_list[-1] - target_log_prob_list[-2]
                reward = torch.clamp(reward, min=0)
                if step % cfg.print_freq == 0:
                    print(f"improved accuracy: {reward[:5]}")
                rewards.append(reward.detach())

        if step % cfg.print_freq == 0:
            print(f"target prediction: {mean.flatten()[:5]}")
            print(f"target ground truth: {batch.target_y.flatten()[:5]}")
